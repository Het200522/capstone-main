...
class DiseaseModel:
    ...
    def __init__(self):
        ...
        self.diseases = self.disease_list(os.path.join(self.data_dir, 'dataset.csv'))

    def load_xgboost(self, model_path):
        try:
            # Try to load the model using native XGBoost
            from xgboost import Booster
            booster = Booster()
            booster.load_model(model_path)
            # Create a wrapper model that can be used with predict
            import xgboost as xgb
            self.model = xgb.XGBClassifier()
            ...
            # Continue without loading the model to let the app work
    ...
    def disease_list(self, kaggle_dataset):
        ...
        df = pd.read_csv(os.path.join(self.data_dir, 'clean_dataset.tsv'), sep='\t')
        # Preprocessing
        y_data = df.iloc[:,-1]
        X_data = df.iloc[:,:-1]
