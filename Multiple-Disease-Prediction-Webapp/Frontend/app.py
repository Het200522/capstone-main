...
#     # Find the correct index based on the session state
#     try:
#         default_idx = menu_pages.index(st.session_state.selected_page)
#     except ValueError:
#         default_idx = 0
...
#         menu_pages,
#         icons=["house-heart", "bug", "wind", "lungs", "info-circle"],
#         default_index=default_idx, # <--- DYNAMIC INDEX
#         styles={
#         # ... keep all your existing styles here ...
#         }
#     )
 
#     # Update the session state to match whatever the menu says
...
with st.sidebar:
    ...
    # Render the menu WITH the styles safely inside it!
    selected = option_menu(
        "Navigation",
        menu_pages,
        icons=["house-heart", "bug", "wind", "lungs", "info-circle"],
...
<a href="?page=dengue" target="_self" class="card-button dengue" title="Click to detect Dengue">
<span class="card-icon">🦟</span>
<div class="card-title">Dengue<br>Detection</div>
<div class="card-subtitle">CBC Analysis</div>
</a>
<a href="?page=asthma" target="_self" class="card-button asthma" title="Click to screen Asthma">
<span class="card-icon">🫁</span>
<div class="card-title">Asthma<br>Screening</div>
<div class="card-subtitle">Respiratory Analysis</div>
</a>
<a href="?page=pneumonia" target="_self" class="card-button pneumonia" title="Click to detect Pneumonia">
<span class="card-icon">🔍</span>
<div class="card-title">Pneumonia<br>Detection</div>
<div class="card-subtitle">X-ray Analysis</div>
</a>
<a href="?page=lab" target="_self" class="card-button lab" title="Click to view Lab Reports">
<span class="card-icon">📊</span>
...
<div class="card-subtitle">OCR Extraction</div>
    ...
    if "page" in query:
        target_page = None
        if query["page"] == "dengue":
            target_page = "Dengue Prediction"
        elif query["page"] == "asthma":
            target_page = "Asthma Prediction"
        elif query["page"] == "pneumonia":
            target_page = "Pneumonia Detection"
        elif query["page"] == "lab":
            target_page = "About"
 
        if target_page and target_page != st.session_state.selected_page:
            st.session_state.selected_page = target_page
            # Clear the query params so it doesn't get stuck in a loop
