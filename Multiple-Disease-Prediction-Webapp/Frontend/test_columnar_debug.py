"""
Debug the columnar format extraction
"""
import sys
sys.path.insert(0, '.')
...
# Debug the internal extraction
print("Testing _universal_extraction directly:")
values = CBCReportFormatter._universal_extraction(columnar_report)
print(f"Extracted {len(values)} values:")
for test, val in values.items():
    print(f"  {test}: {val}")

print("\n\nTesting _try_columnar_extraction directly:")
text_lower = columnar_report.lower().replace(",", "").replace("(abs)", " abs").replace("(absolute)", " abs")
lines = text_lower.split("\n")
col_values = CBCReportFormatter._try_columnar_extraction(text_lower, lines)
print(f"Extracted {len(col_values)} values:")
for test, val in col_values.items():
    print(f"  {test}: {val}")
